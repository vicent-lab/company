...
export class SustainabilityAgent {
  ...
  async analyze(question: string): Promise<AgentResult> {
    const [feedRes, waterRes, energyRes] = await Promise.all([
      query(`SELECT COALESCE(SUM(quantity),0) AS total_feed FROM feed_consumption WHERE farm_id=$1 AND consumed_on >= date_trunc('month', current_date)`, [this.knowledge['farmId']]),
      query(`SELECT COALESCE(SUM(quantity),0) AS total_water FROM water_consumption WHERE farm_id=$1 AND consumed_on >= date_trunc('month', current_date)`, [this.knowledge['farmId']]),
    ...
    const totalWater = Number(waterRes.rows[0]?.total_water || 0);
    ...
    evidence.push(`Water consumed this month: ${totalWater.toFixed(1)} L`);
    ...
    recommendedActions.push('Check for water leaks and wastage');
    recommendedActions.push('Consider renewable energy options');

    return {
      agent: 'sustainability',
      title: 'Sustainability metrics',
      summary: `Feed: ${totalFeed.toFixed(1)} units, Water: ${totalWater.toFixed(1)} L, Energy: ${totalEnergy.toFixed(2)} this month.`,
      severity: 'low',
      confidence: 0.7,
      evidence,
      reasoning,
      ...
      data: { totalFeed, totalWater, totalEnergy },
    };
  }
}
